...
`setDeliveryForm` - данные по доставке
`setContactsForm` - данные о контактах
`setCatalog` - каталог товаров
`setPreview` - предпросмотр продукта
`spotProductInBasket` - проверка нахождения товара в корзине
`validateAddress` - проверка валидности формы доставки
`validateOrder` - проверка валидности формы контактов
...
`set price` - устанавливает цену товара и так же производит отключение кнопки покупки если товар без цены
...
`set selected` - уставливает блокировку и разблокировку оформления товара
